...
function Expenses() {
    ...
    //             monthlyTotals[month].credit += parseFloat(transaction.amount);
    //         } else if (transaction.transactionType === 'Debit') {
    //             monthlyTotals[month].debit += parseFloat(transaction.amount);
    //         }
    //     });

    //     const months = Object.keys(monthlyTotals);
    //     const creditAmounts = months.map(month => monthlyTotals[month].credit);
    //     const debitAmounts = months.map(month => monthlyTotals[month].debit);
    ...
    const aggregateMonthlyData = (transactions) => {
        ...
        transactions.forEach(transaction => {
            ...
            if (last12Months.includes(month) && transactionDate >= new Date(currentDate.getFullYear(), currentDate.getMonth() - 11, 1)) {
                if (transaction.transactionType === 'Credit') {
                    monthlyTotals[month].credit += parseFloat(transaction.amount);
                } else if (transaction.transactionType === 'Debit') {
                    monthlyTotals[month].debit += parseFloat(transaction.amount);
